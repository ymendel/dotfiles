...
#
#   1. shell/bash-completion — holding a one-line files for each Homebrew completion
#      completion whose filename isn't the command it completes
#   2. another directory that symlinks to Homebrew's compat dir — handled by install.sh
#
# Most of the work is done by #2 there. Since the vast majority of commands do match,
# and that Homebrew-installed completion does fine. (And it's necessary, since while
# bash-completion ships a large fallback set of its own, that's for standard Unix tools
# and not Homebrew's formulae.)
